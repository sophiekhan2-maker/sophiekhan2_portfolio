...
#Analyse return rates by category and channel.

# Which channels have high return rates
select `channel`,
		count(*) as total_orders,
        sum(case when returned = 'Yes' then 1 else 0 end) as total_returned,
		round(sum(case when returned = 'Yes' then 1 else 0 end)/count(*), 3) as return_rate
...
# How returns impact profit
##Returns reduce profit because BrightCart must refund the customer, absorb shipping and handling 
#costs, and process the returned item. High return rates directly lower net revenue and profit margins. 
#Categories such as Clothing and Electronics, and channels like Marketplace and Social Commerce, show higher return rates, which significantly reduce
# their profitability compared to Website and Mobile App.


#Which categories or channels need improvement?

#BrightCart should reduce spend on Email Marketing and reallocate budget to high‑performing platforms such
# as TikTok, Instagram, Google Ads, and Influencer campaigns. Clothing and Electronics require improvements to reduce return rates,
# while Marketplace and Social Commerce need stricter seller controls and clearer product information. 
#Website and Mobile App remain the most profitable channels and should be prioritised for optimisation and customer retention strategies.
